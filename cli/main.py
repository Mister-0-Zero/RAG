...
def process_query(query: str, pipeline: SimpleNamespace) -> None:
    """
    Processes a single user query through the RAG pipeline and logs the results.
    ...
    for r in reranked_results:
        ...
        log_result_details(r, context_chunks)
        
        compressed_context = pipeline.compressor.compress(question=query, chunks=context_chunks)
        final_context.append({**r, "compressed_context": compressed_context})

    # 4. Answering
    answer_result = pipeline.answer_generator.generate(query=query, final=final_context)
    log_answer(answer_result)

def log_result_details(result: dict, context_chunks: list) -> None:
    """Logs the detailed information of a single retrieval result."""
    score = result.get("score", 0.0)
    rerank_score = result.get("rerank_score", 0.0)
    doc_name = result['main_chunk'].doc_name
    
    log.info(
        f"Found source: {doc_name} (Score: {score:.4f}, Rerank Score: {rerank_score:.4f})",
        extra={'log_type': 'MODEL_RESPONSE'}
    )
    context_text = " ".join([c.text for c in context_chunks])
    log.info(f"Context before compression: {context_text}", extra={'log_type': 'DEBUG'})

def log_answer(answer_result: AnswerResult) -> None:
    """Logs the final answer and citations."""
    log.info("=== FINAL ANSWER ===", extra={'log_type': 'MODEL_RESPONSE'})
    log.info(answer_result.answer, extra={'log_type': 'MODEL_RESPONSE'})
    if answer_result.citations:
        log.info(
            f"Sources: {', '.join(answer_result.citations)}",
